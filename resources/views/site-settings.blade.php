                ...
                <form action="/admin/site-settings" method="post">
                  ...
                  <div class="mb-5.5 flex flex-col gap-5.5 sm:flex-row">
                    <div class="p-2 w-full sm:w-1/2">
                      ...
                      <div class="relative">
                        <input value="{{old('workdayStart') ?? $configs['calendarTimes']['slotMinTime']}}" type="time" name="workdayStart" id="workdayStart" placeholder="Workday Start" class="w-full rounded border border-stroke bg-gray py-3 pl-11.5 pr-4.5 font-medium text-black focus:border-primary focus-visible:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                        @error('workdayStart')
                            {{$message}}
                        @enderror
                    </div>
                    ...
                    <div class="p-2 w-full sm:w-1/2">
                      <label class="mb-3 block text-sm font-medium text-black dark:text-white" for="workdayEnd">Workday End</label>
                          <div class="relative">
                              <input value="{{old('workdayEnd') ?? $configs['calendarTimes']['slotMaxTime']}}" type="time" name="workdayEnd" id="workdayEnd" placeholder="Workday End" class="w-full rounded border border-stroke bg-gray py-3 pl-11.5 pr-4.5 font-medium text-black focus:border-primary focus-visible:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                                @error('workdayEnd')
                                    {{$message}}
                                @enderror
                            </div>
            ...
            <div class="rounded-sm border border-stroke bg-white shadow-default dark:bg-gray-800 dark:border-gray-800 dark:text-white">
              <div class="p-7">
                <form action="/admin/site-settings/fill-holidays" method="post">
                  ...
                  <input type="number" min="2020" max="2100" step="1" name="year" value="{{date('Y')}}" class="w-full rounded border border-stroke bg-gray py-3 pl-11.5 pr-4.5 font-medium text-black focus:border-primary focus-visible:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"/>
                  @error('year')
                    {{$message}}
                  @enderror
